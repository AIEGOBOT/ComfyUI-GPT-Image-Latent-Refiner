...
### 4. SeedVR2 VAE 인코드·디코드와 메모리

예제 JSON을 기준으로 SeedVR2 단계는 `ema_vae_fp16.safetensors`를 불러와 일반
`VAEEncode`와 `VAEDecode` 노드로 처리합니다. 기본 그래프가 타일 VAE 노드를
사용한다고 오해하면 안 됩니다.

리파이너 노드의 `tile_vae`는 리파이너 내부 Diffusers VAE에만 적용됩니다. SeedVR2
VAE 인코드·디코드는 별도의 후속 단계이므로 그 스위치의 영향을 받지 않습니다.
SeedVR2 VAE 단계에서 OOM이 발생할 때만 일반 노드를 ComfyUI의 타일 VAE 인코드와
디코드 노드로 교체하세요. 타일 처리는 메모리를 줄이지만 더 느릴 수 있고 드물게
경계 차이가 보일 수 있습니다.

리파이너 단독 실행과 7B SeedVR2 결합 실행의 VRAM 요구량도 분리해서 판단해야
합니다. 결합 그래프는 목표 해상도, SeedVR2 모델 정밀도, 오프로딩 상태와 VAE 처리
방식의 영향을 크게 받습니다. 먼저 배치 1장과 낮은 목표 해상도로 검증하세요.
